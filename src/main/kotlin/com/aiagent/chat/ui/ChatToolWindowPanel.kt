...
class ChatToolWindowPanel(private val project: Project) : JBPanel<ChatToolWindowPanel>(CardLayout()) {
    ...
    // --- Multi-tier model manager (cognitive tier model assignment) ---
    private val modelTierManager = com.aiagent.chat.model.ModelTierManager(providerManager)
    ...
    private fun executePrompt(promptText: String, displayText: String = promptText, referencedFiles: List<String> = emptyList()) {
        ...
        activeEngineJob = scope.launch {
            ...
            // --- Multi-tier model configuration: load tier config and update manager ---
            if (settings.isModelTierEnabled()) {
                modelTierManager.maxOutputTokens = if (settings.state.maxOutputTokens > 0) settings.state.maxOutputTokens else null
                modelTierManager.updateConfiguration(settings.getModelTierConfig())
                modelTierManager.clearCache()
                DebugLog.info("ChatToolWindow", "Multi-tier model configuration enabled: ${modelTierManager.configuration.tiers.filter { it.modelDisplayName.isNotBlank() }.joinToString { "${it.tier.name}=${it.modelDisplayName}" }}")
            }
            ...
            val engine = AgentEngine(
                ...
                modelTierManager = if (settings.isModelTierEnabled()) modelTierManager else null,
                onDelta = { delta ->
                    when (delta) {
